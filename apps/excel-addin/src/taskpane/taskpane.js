...
function createWorkbookStructureMoveLocalExecutionSnapshot({
  ...
  if (
    !executionId ||
    !sheetName ||
    !Number.isInteger(beforePosition) ||
    !Number.isInteger(afterPosition) ||
...
function createWorkbookStructureDuplicateLocalExecutionSnapshot({
  executionId,
  sourceSheetName,
  copiedSheetName,
  sourcePosition,
  ...
  if (!executionId || !sourceSheetName || !copiedSheetName) {
    return null;
  }
  ...
  return {
    ...
    source: {
      ...
      ...(Number.isInteger(sourcePosition) ? { position: sourcePosition } : {})
    },
    before: {
      exists: false,
      name: copiedSheetName
    },
    after: {
      exists: true,
      name: copiedSheetName,
      ...(Number.isInteger(copiedPosition) ? { position: copiedPosition } : {})
    }
  };
}
...
function assertWorkbookStructureDuplicateSourceSnapshotState(state) {
  if (
    !state ||
    state.exists !== true ||
    typeof state.name !== "string" ||
...
function assertWorkbookStructureDuplicatePresenceSnapshotState(state) {
  if (
    !state ||
    typeof state.exists !== "boolean" ||
    typeof state.name !== "string" ||
...
async function applyWorkbookStructureSnapshotTransition(context, transition) {
  ...
  if (transition.operation === "duplicate_sheet") {
    assertWorkbookStructureDuplicateSourceSnapshotState(transition.source);
    assertWorkbookStructureDuplicatePresenceSnapshotState(transition.from);
    assertWorkbookStructureDuplicatePresenceSnapshotState(transition.to);
    ...
    if (transition.from.exists === true && transition.to.exists === false) {
      ...
      if (
        Number.isInteger(transition.from.position) &&
    ...
    if (transition.from.exists === false && transition.to.exists === true) {
      ...
      const orderedSheets = Array.isArray(worksheets.items)
        ? [...worksheets.items].sort((left, right) => (left.position || 0) - (right.position || 0))
        : [];
      const sourceSheet = worksheets.getItem(transition.source.name);
      if (typeof sourceSheet.load === "function") {
        sourceSheet.load(["name", "position"]);
        await context.sync();
      }
      if (typeof sourceSheet.copy !== "function") {
        throw new Error("Excel host does not support duplicating this worksheet.");
      }

      const desiredPosition = Number.isInteger(transition.to.position)
        ? Math.max(0, transition.to.position)
        : orderedSheets.length;
      let copiedSheet;
      if (desiredPosition <= 0) {
        copiedSheet = sourceSheet.copy(Excel.WorksheetPositionType.beginning);
      } else if (desiredPosition >= orderedSheets.length || orderedSheets.length === 0) {
      ...
      copiedSheet.name = transition.to.name;
      return;
    }
  ...
  if (transition.operation === "move_sheet") {
    ...
    sheet.position = transition.to.position;
    return;
  }
...
async function validateWorkbookStructureLocalExecutionSnapshotForMode(snapshot, mode) {
  ...
  return Excel.run(async (context) => {
    if (transition.operation === "create_sheet") {
      ...
      assertWorkbookStructureMoveSnapshotState(transition.from);
      assertWorkbookStructureMoveSnapshotState(transition.to);
    } else if (transition.operation === "duplicate_sheet") {
      assertWorkbookStructureDuplicateSourceSnapshotState(transition.source);
      assertWorkbookStructureDuplicatePresenceSnapshotState(transition.from);
      assertWorkbookStructureDuplicatePresenceSnapshotState(transition.to);
    } else {
    ...
    const sheetName = transition.operation === "duplicate_sheet" && transition.from.exists === false
    ...
    if (typeof sheet.load === "function") {
      sheet.load(transition.operation === "sheet_visibility"
        ...
        : transition.operation === "move_sheet"
          ? ["name", "position"]
          : transition.operation === "duplicate_sheet"
            ? ["name", "position", "visibility"]
          : ["name"]);
      await context.sync();
    }
...
function containsNumericIpv4Url(value) {
...
function containsSensitiveHostDiagnostics(message) {
  ...
  return (
    ...
    /\b(?:[A-Z][A-Z0-9_]*(?:SECRET|TOKEN|PASSWORD|PRIVATE|CREDENTIAL|API_KEY|SERVER_KEY|BASE_URL)[A-Z0-9_]*|SECRET|TOKEN|PASSWORD|PRIVATE|CREDENTIAL|API_KEY|SERVER_KEY|BASE_URL)\b/.test(text) ||
...
const UNSAFE_PROOF_METADATA_PATTERN = /(?:HERMES_[A-Z0-9_]+|[A-Z][A-Z0-9_]*(?:SECRET|TOKEN|PASSWORD|PRIVATE|CREDENTIAL|API_KEY|SERVER_KEY|BASE_URL)[A-Z0-9_]*|client_secret|refresh_token|access_token|authorization|api[_-]?key|approval_secret|stack trace|traceback|ReferenceError|TypeError|SyntaxError|RangeError)|(?:^|[\s(["'=:])\/(?:root|srv|home|tmp|var|opt|workspace|app|mnt|Users)\/[^\s]+|(?:^|[\s(["'=:])[A-Za-z]:\\[^\s]+|(?:^|[\s(["'=:])\\\\[^\s]+|https?:\/\/(?:internal(?:[.\w-]*)?|localhost|127\.0\.0\.1|10\.\d{1,3}\.\d{1,3}\.\d{1,3}|169\.254\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|172\.(?:1[6-9]|2\d|3[0-1])\.\d{1,3}\.\d{1,3}|\[(?:::ffff:|(?:0:){5}ffff:)(?:(?:127|10)(?:\.\d{1,3}){3}|169\.254\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|172\.(?:1[6-9]|2\d|3[01])\.\d{1,3}\.\d{1,3}|(?:0{1,4}|7f[0-9a-f]{2}|0?a[0-9a-f]{2}|a9fe|c0a8|ac1[0-9a-f]):[0-9a-f]{1,4})\]|\[(?:::|::1|f[cd][0-9a-f:]*|fe[89ab][0-9a-f:]*)\])[^\s]*/i;
...
function containsSensitiveGatewayErrorText(text) {
  ...
  return (
    ...
    /\b(?:[A-Z][A-Z0-9_]*(?:SECRET|TOKEN|PASSWORD|PRIVATE|CREDENTIAL|API_KEY|SERVER_KEY|BASE_URL)[A-Z0-9_]*|SECRET|TOKEN|PASSWORD|PRIVATE|CREDENTIAL|API_KEY|SERVER_KEY|BASE_URL)\b/.test(value) ||
      ...
      switch (resolvedPlan.operation) {
        ...
        case "duplicate_sheet": {
          ...
          return attachLocalExecutionSnapshot({
            ...
            sheetCount: orderedSheets.length + 1,
            summary: getWorkbookStructureStatusSummary(resolvedPlan)
          }, createWorkbookStructureDuplicateLocalExecutionSnapshot({
            executionId,
            sourceSheetName: sourceSheet.name || resolvedPlan.sheetName,
            copiedSheetName,
            sourcePosition: sourceSheet.position,
            copiedPosition: desiredPosition
          }));
        }
        case "move_sheet": {
          const sheet = findSheet(resolvedPlan.sheetName);
          ...
          const beforePosition = sheet.position;
          const desiredPosition = clampExistingPosition(resolvedPlan.position, orderedSheets.length);
          sheet.position = desiredPosition;
          await context.sync();
          const afterPosition = Number.isInteger(sheet.position) ? sheet.position : desiredPosition;
          return attachLocalExecutionSnapshot({
            kind: "workbook_structure_update",
            hostPlatform: platform,
            sheetName: resolvedPlan.sheetName,
            ...
            summary: getWorkbookStructureStatusSummary(resolvedPlan)
          }, createWorkbookStructureMoveLocalExecutionSnapshot({
            executionId,
            sheetName: resolvedPlan.sheetName,
            beforePosition,
            afterPosition
