...
function createRangeStub(options: {
  ...
  const range = {
    ...
    copyFrom: vi.fn((source: { __getFormatCells?: () => ReturnType<typeof cloneFormatCells>; __getBorders?: () => typeof currentBorders }, copyType: string, skipBlanks: boolean, transpose: boolean) => {
      copyFromCalls.push({
        from: [0, 0],
        to: [0, 0],
        copyType,
...
describe("Excel wave 4 transfer and cleanup plans", () => {
  ...
  it("attaches an undo snapshot for Excel value append transfers", async () => {
    const sourceRange = createRangeStub({
      address: "RawData!A2:B3",
      rowCount: 2,
      columnCount: 2,
    ...
    const result = await taskpane.applyWritePlan({
      plan: {
        ...
        pasteMode: "values",
        transpose: false,
        explanation: "Append finalized rows into the archive block.",
        confidence: 0.93,
        requiresConfirmation: true,
        affectedRanges: ["RawData!A2:B3", "Archive!D5:E6"],
        overwriteRisk: "medium",
      ...
      requestId: "req_range_transfer_append_undo_excel_001",
      runId: "run_range_transfer_append_undo_excel_001",
      approvalToken: "token",
      executionId: "exec_range_transfer_append_undo_excel_001"
    });
    ...
    expect(result).toMatchObject({
      ...
      transpose: false,
      __hermesLocalExecutionSnapshot: {
        baseExecutionId: "exec_range_transfer_append_undo_excel_001",
        targetSheet: "Archive",
        targetRange: "D5:E6",
        beforeCells: [
          [{ kind: "value", value: { type: "string", value: "Old" } }, { kind: "value", value: { type: "string", value: "Target" } }],
  ...
  it("applies remove_duplicate_rows cleanup across formula-containing ranges in Excel", async () => {
    const targetRange = createRangeStub({
      address: "Contacts!A2:B6",
      rowCount: 5,
      columnCount: 2,
    ...
    await expect(taskpane.applyWritePlan({
      ...
      requestId: "req_cleanup_remove_duplicate_rows_formulas_excel_001",
      runId: "run_cleanup_remove_duplicate_rows_formulas_excel_001",
      approvalToken: "token"
    })).resolves.toMatchObject({
      kind: "data_cleanup_update",
      operation: "remove_duplicate_rows",
