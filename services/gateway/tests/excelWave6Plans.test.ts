...
describe("Excel wave 6 composite plans and execution controls", () => {
  it("sanitizes host execution failures into user-facing guidance", async () => {
    ...
    expect(taskpane.sanitizeHostExecutionError(
      new Error(String.raw`Unhandled writeback failure at ("C:\Users\runner\work\host.ts:42")`)
    )).toBe("Write-back failed.");
  ...
  it("restores Excel sheet visibility snapshots before committing undo", async () => {
    const workbookSessionId = "workbook-123";
    const workbookSessionKey = `excel_windows::${workbookSessionId}`;
    const snapshotStoreKey = `Hermes.ReversibleExecutions.v1::${workbookSessionKey}`;
    const localSnapshotStore = JSON.stringify({
    ...
    await taskpane.undoExecution("exec_hide_sheet_001");
  ...
  it("restores Excel row visibility snapshots before committing undo", async () => {
    const workbookSessionId = "workbook-123";
    const workbookSessionKey = `excel_windows::${workbookSessionId}`;
    const snapshotStoreKey = `Hermes.ReversibleExecutions.v1::${workbookSessionKey}`;
    const localSnapshotStore = JSON.stringify({
      version: 1,
      order: ["exec_hide_rows_001"],
      executions: {
        exec_hide_rows_001: {
          baseExecutionId: "exec_hide_rows_001"
        }
      },
      bases: {
        exec_hide_rows_001: {
    ...
    await taskpane.undoExecution("exec_hide_rows_001");

    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(fetchMock.mock.calls[0][0]).toBe("http://127.0.0.1:8787/api/execution/undo/prepare");
    expect(fetchMock.mock.calls[1][0]).toBe("http://127.0.0.1:8787/api/execution/undo");
    expect(rowHidden.get("3:3")).toBe(false);
  ...
  it("restores Excel freeze pane snapshots before committing undo", async () => {
    const workbookSessionId = "workbook-123";
    const workbookSessionKey = `excel_windows::${workbookSessionId}`;
    const snapshotStoreKey = `Hermes.ReversibleExecutions.v1::${workbookSessionKey}`;
    const localSnapshotStore = JSON.stringify({
      version: 1,
      order: ["exec_freeze_panes_001"],
      executions: {
        exec_freeze_panes_001: {
          baseExecutionId: "exec_freeze_panes_001"
        }
      },
      bases: {
        exec_freeze_panes_001: {
    ...
    let frozenRangeAddress = "Sheet1!B3";
    const freezePanes = {
      ...
      freezeAt: vi.fn((range: string) => {
        frozenRangeAddress = `Sheet1!${range}`;
      }),
      unfreeze: vi.fn(() => {
        frozenRangeAddress = "";
      })
    ...
    await taskpane.undoExecution("exec_freeze_panes_001");

    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(fetchMock.mock.calls[0][0]).toBe("http://127.0.0.1:8787/api/execution/undo/prepare");
    expect(fetchMock.mock.calls[1][0]).toBe("http://127.0.0.1:8787/api/execution/undo");
    expect(freezePanes.freezeAt).toHaveBeenCalledWith("A2");
    expect(frozenRangeAddress).toBe("Sheet1!A2");
  });
  ...
  it("restores Excel sheet create snapshots before committing undo", async () => {
    ...
    const localSnapshotStore = JSON.stringify({
      version: 1,
      order: ["exec_create_sheet_001"],
      executions: {
        exec_create_sheet_001: {
          baseExecutionId: "exec_create_sheet_001"
        }
      },
      bases: {
        exec_create_sheet_001: {
    ...
    await taskpane.undoExecution("exec_create_sheet_001");

    expect(fetchMock).toHaveBeenCalledTimes(2);
    expect(fetchMock.mock.calls[0][0]).toBe("http://127.0.0.1:8787/api/execution/undo/prepare");
    expect(fetchMock.mock.calls[1][0]).toBe("http://127.0.0.1:8787/api/execution/undo");
    expect(worksheet.delete).toHaveBeenCalledTimes(1);
  });

  it("restores Excel sheet move snapshots before committing undo", async () => {
    const workbookSessionId = "workbook-123";
    const workbookSessionKey = `excel_windows::${workbookSessionId}`;
    const snapshotStoreKey = `Hermes.ReversibleExecutions.v1::${workbookSessionKey}`;
    const localSnapshotStore = JSON.stringify({
      version: 1,
      order: ["exec_move_sheet_001"],
      executions: {
        exec_move_sheet_001: {
          baseExecutionId: "exec_move_sheet_001"
        }
      },
      bases: {
        exec_move_sheet_001: {
    ...
    await taskpane.undoExecution("exec_move_sheet_001");
