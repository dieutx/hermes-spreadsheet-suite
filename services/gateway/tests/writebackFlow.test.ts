...
describe("writeback confirmation flow", () => {
  ...
  it("keeps chart history non-undoable until the host confirms an exact rollback snapshot", () => {
    const traceBus = new TraceBus();
    const executionLedger = new ExecutionLedger();
    const plan = {
      sourceSheet: "Sales",
    ...
    setRunResponse(traceBus, {
      runId: "run_chart_history_001",
      requestId: "req_chart_history_001",
      type: "chart_plan",
      traceEvent: "chart_plan_ready",
      plan
    });
  ...
  it("records chart history as undo-eligible when the host confirms an exact rollback snapshot", () => {
    const traceBus = new TraceBus();
    const executionLedger = new ExecutionLedger();
    const plan = {
      sourceSheet: "Sales",
    ...
    setRunResponse(traceBus, {
      runId: "run_chart_history_undo_ready",
      requestId: "req_chart_history_undo_ready",
      type: "chart_plan",
      traceEvent: "chart_plan_ready",
      plan
    });
  ...
  it("records data validation history as undo-eligible when the host confirms an exact rollback snapshot", () => {
    const traceBus = new TraceBus();
    const executionLedger = new ExecutionLedger();
    const plan = {
      targetSheet: "Sheet1",
  ...
  it("records named range history as undo-eligible when the host confirms an exact rollback snapshot", () => {
    const traceBus = new TraceBus();
    const executionLedger = new ExecutionLedger();
    const plan = {
      operation: "retarget",
    ...
    setRunResponse(traceBus, {
      runId: "run_named_range_undo_ready",
      requestId: "req_named_range_undo_ready",
      type: "named_range_update",
      traceEvent: "named_range_update_ready",
      plan
    });
  ...
  it("records sheet structure history as undo-eligible when the host confirms an exact rollback snapshot", () => {
    const traceBus = new TraceBus();
    const executionLedger = new ExecutionLedger();
    const plan = {
      targetSheet: "Sheet1",
    ...
    setRunResponse(traceBus, {
      runId: "run_sheet_structure_undo_ready",
      requestId: "req_sheet_structure_undo_ready",
      type: "sheet_structure_update",
      traceEvent: "sheet_structure_update_ready",
      plan
    });
    ...
    expect(
      ...
      planType: "sheet_structure_update",
      reversible: true,
      undoEligible: true,
      redoEligible: false
    });
  ...
  it("keeps external data history non-undoable until the host confirms an exact rollback snapshot", () => {
    const traceBus = new TraceBus();
    const executionLedger = new ExecutionLedger();
    const plan = {
      sourceType: "market_data",
    ...
    setRunResponse(traceBus, {
      runId: "run_external_data_history_001",
      requestId: "req_external_data_history_001",
      type: "external_data_plan",
      traceEvent: "external_data_plan_ready",
      plan
    });
    ...
    expect(
      ...
      planType: "external_data_plan",
      reversible: true,
      undoEligible: false
    });
  });

  it("records external data history as undo-eligible when the host confirms an exact rollback snapshot", () => {
    const traceBus = new TraceBus();
    const executionLedger = new ExecutionLedger();
    const plan = {
      sourceType: "market_data",
    ...
    setRunResponse(traceBus, {
      runId: "run_external_data_history_undo_ready",
      requestId: "req_external_data_history_undo_ready",
      type: "external_data_plan",
      traceEvent: "external_data_plan_ready",
      plan
    });
