...
describe("HermesAgentClient", () => {
    ...
    expect(() =>
      HermesStructuredBodySchema.parse(normalizeHermesStructuredBodyInput({
        ...
        data: {
          ...
          confidence: 0.91,
          requiresConfirmation: true,
          affectedRanges: ["Marketing!A15:D20", "Marketing!F15"],
          overwriteRisk: "low",
          confirmationLevel: "destructive",
          reversible: false,
  ...
  it("sanitizes provider error diagnostics before returning gateway error responses", async () => {
    process.env.HERMES_AGENT_BASE_URL = "http://agent.test/v1";
    const client = new HermesAgentClient(getConfig());
    const traceBus = new TraceBus();
    ...
    await client.processRequest({
      runId: "run_provider_diagnostic_sanitized_001",
      request: baseRequest({
        requestId: "req_provider_diagnostic_sanitized_001",
        userMessage: "Explain this workbook."
      }),
      traceBus
    });

    const response = traceBus.getRun("run_provider_diagnostic_sanitized_001")?.response;
    expect(response?.type).toBe("error");
    expect(response?.data).toMatchObject({
      code: "PROVIDER_ERROR",
  ...
  it("sanitizes embedded provider error diagnostics before returning gateway error responses", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => new Response(JSON.stringify({
      error: {
        message: "Provider failed for qa_HERMES_API_SERVER_KEY"
      }
    }), {
      status: 503,
      headers: { "content-type": "application/json" }
    ...
    await client.processRequest({
      runId: "run_provider_embedded_diagnostic_sanitized_001",
      request: baseRequest({
        requestId: "req_provider_embedded_diagnostic_sanitized_001",
        userMessage: "Explain this workbook."
      }),
      traceBus
    });

    const response = traceBus.getRun("run_provider_embedded_diagnostic_sanitized_001")?.response;
    expect(response?.type).toBe("error");
    expect(response?.data).toMatchObject({
      code: "PROVIDER_ERROR",
      message: "The Hermes service couldn't complete that request right now.",
  ...
  it("sanitizes IPv4-mapped private provider diagnostics before returning gateway error responses", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => new Response(JSON.stringify({
      error: {
        message: "Provider failed while fetching http://[::ffff:7f00:1]/latest/meta-data"
      }
    }), {
      status: 503,
      headers: { "content-type": "application/json" }
    ...
    await client.processRequest({
      runId: "run_provider_mapped_diagnostic_sanitized_001",
      request: baseRequest({
        requestId: "req_provider_mapped_diagnostic_sanitized_001",
        userMessage: "Explain this workbook."
      }),
      traceBus
    });

    const response = traceBus.getRun("run_provider_mapped_diagnostic_sanitized_001")?.response;
    expect(response?.type).toBe("error");
    expect(response?.data).toMatchObject({
      code: "PROVIDER_ERROR",
      message: "The Hermes service couldn't complete that request right now.",
    ...
    expect(JSON.stringify(response)).not.toContain("::ffff");
  ...
  it("returns provider errors when non-2xx provider JSON is malformed", async () => {
    ...
    await client.processRequest({
      runId: "run_provider_malformed_json_001",
      request: baseRequest({
        requestId: "req_provider_malformed_json_001",
        userMessage: "Explain this workbook."
      }),
      traceBus
    });

    const response = traceBus.getRun("run_provider_malformed_json_001")?.response;
    expect(response?.type).toBe("error");
    expect(response?.data).toMatchObject({
      code: "PROVIDER_ERROR",
      message: "Hermes Agent request failed with status 502.",
      retryable: true,
      userAction: "Retry the request after the remote Hermes Agent service recovers."
    });
  });
