...
function createRangeStub(options: {
  ...
  const range = {
    ...
    copyTo: vi.fn((destination: { __setFormatState?(nextFormat: typeof currentFormat): void }, pasteType: string, transposed: boolean) => {
      copyToCalls.push({
...
describe("Google Sheets wave 4 transfer and cleanup plans", () => {
  ...
  it("attaches an undo snapshot for Google Sheets value append transfers", () => {
    const sourceRange = createRangeStub({
      a1Notation: "A2:B3",
      row: 2,
      column: 1,
    ...
    const result = applyWritePlan({
      ...
      executionId: "exec_range_transfer_append_undo_sheets_001"
    });
    ...
    expect(result).toMatchObject({
      ...
      transpose: false,
      __hermesLocalExecutionSnapshot: {
        baseExecutionId: "exec_range_transfer_append_undo_sheets_001",
        targetSheet: "Archive",
        targetRange: "D5:E6",
        beforeCells: [
          [{ kind: "value", value: { type: "string", value: "Old" } }, { kind: "value", value: { type: "string", value: "Target" } }],
    ...
    expect(sourceRange.clearContent).not.toHaveBeenCalled();
    expect(flush).toHaveBeenCalledTimes(1);
  });
  ...
  it("fails closed for unsupported formula-aware standardize_format plans when the target range contains formulas in Google Sheets", () => {
    ...
    expect(() => applyWritePlan({
      plan: {
        ...
        formatType: "date",
        formatPattern: "yyyy-mm-dd",
        explanation: "Standardize formula-driven dates in place.",
        confidence: 0.82,
        requiresConfirmation: true,
        affectedRanges: ["Contacts!A2:C4"],
        overwriteRisk: "medium",
