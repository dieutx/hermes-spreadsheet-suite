  ...
  switch (request.host.platform) {
    case "google_sheets":
      return [
        ...
        "- chart_plan: limited. Require a single-cell target anchor and categoryField. Supported chart types: bar, column, stacked_bar, stacked_column, line, area, pie, scatter. Series labels may use explicit custom legend text when provided. Supported legend positions: bottom, left, right, top, hidden. Optional horizontalAxisTitle and verticalAxisTitle are supported for charts with axes, but not pie charts. If you would naturally say none, emit hidden.",
    ...
    case "excel_macos":
      return [
        ...
        helperSheetLine,
        "- pivot_table_plan: limited. Require a single-cell target anchor. Supported value aggregations: sum, count, average, min, max. Supported pivot sorting: group_field on an existing row or column group, and aggregated_value only when the pivot does not mix row and column groups. Supported pivot filters use existing pivot fields with operators equal_to, not_equal_to, greater_than, greater_than_or_equal_to, less_than, or less_than_or_equal_to.",
        "- chart_plan: limited. Require a single-cell target anchor, categoryField, and at least one unique series field. Supported chart types: bar, column, stacked_bar, stacked_column, line, area, pie, scatter. Series labels must match their source field. Supported legend positions: top, bottom, left, right, hidden. Optional horizontalAxisTitle and verticalAxisTitle are supported for charts with axes, but not pie charts. Pie charts support exactly one series. sourceRange must expose a header row whose field order matches categoryField followed by the requested series fields exactly.",
...
export function buildHermesSpreadsheetRequestPrompt(request: HermesRequest): string {
  ...
  return [
    ...
    "If type=\"chart_plan\", data.sourceSheet, data.sourceRange, data.targetSheet, data.targetRange, data.chartType, data.series, data.explanation, data.confidence, data.requiresConfirmation, data.affectedRanges, data.overwriteRisk, and data.confirmationLevel are required. chartType must be bar, column, stacked_bar, stacked_column, line, area, pie, or scatter. Each series item must use field to reference a source header name. Do not use A1 ranges or name/range objects inside data.series. Use categoryField when the chart should use a named category axis. Use optional horizontalAxisTitle and verticalAxisTitle for axis titles only when the selected chart type has axes; do not use axis titles with pie charts. If the requested chart cannot be mapped exactly on the current host, return type=\"error\" with data.code=\"UNSUPPORTED_OPERATION\".",
