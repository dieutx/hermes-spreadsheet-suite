...
function looksLikeNamedRangeIdentifier(value: string): boolean {
  return /^[A-Za-z_][A-Za-z0-9_]*$/.test(value) &&
    ...
    !/^[RC]$/i.test(value) &&
    !/^[A-Z]{1,3}[1-9][0-9]*$/i.test(value) &&
    !/^R[1-9][0-9]*C[1-9][0-9]*$/i.test(value);
}
  ...
  switch (request.host.platform) {
    case "google_sheets":
      return [
        ...
        "- external_data_plan: limited. Require a single-cell target anchor. Supported sourceType/provider pairs: market_data/googlefinance and web_table_import/importhtml, importxml, or importdata. Formula text must match the provider exactly. Market-data formulas must use literal GOOGLEFINANCE arguments that match data.query fields when present. Web imports require data.sourceUrl to be a public HTTP(S) URL and must not assume authenticated or JavaScript-rendered pages. Web import formulas must use literal arguments matching data.sourceUrl, data.selectorType, and data.selector, and must not reference private or internal URLs.",
        ...
        "- data_cleanup_plan: limited. Supported cleanup operations: trim_whitespace, normalize_case, remove_blank_rows, remove_duplicate_rows, split_column, join_columns, fill_down, and standardize_format. Formula-aware cleanup is exact-safe for formula-containing ranges when host cell-level formula copy and snapshot access is available. normalize_case only supports upper, lower, title, and sentence. standardize_format only supports exact year-first date text patterns and fixed-decimal number text patterns.",
        "- range_filter_plan: limited. Use combiner=and only. topN filters are supported when display values cleanly separate visible and hidden rows. Use positive whole-number topN values only. Duplicate display values crossing the top-N cutoff are unsupported. Repeated conditions on the same column are unsupported.",
...
export function buildHermesSpreadsheetRequestPrompt(request: HermesRequest): string {
  ...
  return [
    ...
    "If type=\"formula\", data.intent, data.formula, data.formulaLanguage, data.explanation, and data.confidence are required. formula response formula fields must start with =. Use data.formulaLanguage=\"excel\" for excel_windows/excel_macos and \"google_sheets\" for google_sheets.",
    ...
    "If type=\"conditional_format_plan\", data.targetSheet, data.targetRange, data.explanation, data.confidence, data.requiresConfirmation, data.affectedRanges, data.replacesExistingRules, and data.managementMode are required. conditional_format_plan is distinct from range_format_update. clear_on_target must contain no rule payload. replace_all_on_target removes existing target rules before applying the new rule. Do not return a vague highlight plan with only explanation and ranges. Include a full contract-valid rule payload. Conditional format style supports backgroundColor, textColor, bold, italic, underline, and strikethrough only; use range_format_update for static numberFormat changes. number_compare values must be finite numbers. date_compare values must be valid YYYY-MM-DD literals. For color_scale, points are ordered minimum, optional midpoint, maximum: do not put max first, min last, or min/max at the midpoint. percent and percentile color_scale point values must be between 0 and 100. For row-highlighting logic driven by a status/breach/overdue column or by comparisons between columns, prefer ruleType=\"custom_formula\" with an exact formula instead of static row-by-row formatting. custom_formula conditional format formulas must start with =. If the requested conditional formatting cannot be mapped exactly on the current host, return type=\"error\" with data.code=\"UNSUPPORTED_OPERATION\".",
    ...
    "If type=\"range_filter_plan\", data.targetSheet, data.targetRange, data.hasHeader, data.conditions, data.combiner, data.clearExistingFilters, data.explanation, data.confidence, and data.requiresConfirmation must be true. data.conditions must include one or more filter conditions and data.combiner must be \"and\".",
    "If type=\"data_validation_plan\", data.targetSheet, data.targetRange, data.ruleType, data.allowBlank, data.invalidDataBehavior, data.explanation, data.confidence, and data.requiresConfirmation must be true. For list validation, use exactly one source: values, sourceRange, or namedRangeName. For checkbox validation, do not include uncheckedValue unless checkedValue is also present. custom_formula validation formulas must start with =. Optional inputTitle, inputMessage, errorTitle, and errorMessage customize validation prompts and invalid-entry alerts when the current host can apply them exactly.",
    "If type=\"analysis_report_plan\", data.sourceSheet, data.sourceRange, data.outputMode, data.sections, data.explanation, data.confidence, data.affectedRanges, data.overwriteRisk, and data.confirmationLevel are required. analysis_report_plan affectedRanges must include the qualified source range; materialize_report must also include the full materialized target range. data.sections must be an array of objects with type, title, summary, and sourceRanges. Do not use plain strings or slugs for report sections. section sourceRanges must be qualified A1 ranges such as Sheet1!A1:D20. data.outputMode must be chat_only or materialize_report. chat_only requires data.requiresConfirmation=false and must not ask for confirmation. materialize_report requires data.targetSheet, data.targetRange, and data.requiresConfirmation=true. For materialize_report, data.targetRange must be the full 4-column destination rectangle for the report matrix, never just the anchor cell. If the requested report artifact cannot be mapped exactly on the current host, return type=\"error\" with data.code=\"UNSUPPORTED_OPERATION\".",
    "If type=\"pivot_table_plan\", data.sourceSheet, data.sourceRange, data.targetSheet, data.targetRange, data.rowGroups, data.valueAggregations, data.explanation, data.confidence, data.requiresConfirmation, data.affectedRanges, data.overwriteRisk, and data.confirmationLevel are required. Pivot comparison filters require numeric values; between and not_between also require value2. Optional sort must use sortOn=\"group_field\" on an existing row or column group, or sortOn=\"aggregated_value\" only when the pivot does not mix row and column groups. If the requested pivot table cannot be mapped exactly on the current host, return type=\"error\" with data.code=\"UNSUPPORTED_OPERATION\".",
    "If type=\"chart_plan\", data.sourceSheet, data.sourceRange, data.targetSheet, data.targetRange, data.chartType, data.categoryField, data.series, data.explanation, data.confidence, data.requiresConfirmation, data.affectedRanges, data.overwriteRisk, and data.confirmationLevel are required. chartType must be bar, column, stacked_bar, stacked_column, line, area, pie, or scatter. categoryField must reference the source header used for the category axis. Each series item must use field to reference a source header name. categoryField and all data.series fields must be unique. Do not use A1 ranges or name/range objects inside data.series. Use optional horizontalAxisTitle and verticalAxisTitle for axis titles only when the selected chart type has axes; do not use axis titles with pie charts. If the requested chart cannot be mapped exactly on the current host, return type=\"error\" with data.code=\"UNSUPPORTED_OPERATION\".",
    "If type=\"table_plan\", data.targetSheet, data.targetRange, data.hasHeaders, data.explanation, data.confidence, data.requiresConfirmation, data.affectedRanges, data.overwriteRisk, and data.confirmationLevel are required. data.name, data.styleName, data.showBandedRows, data.showBandedColumns, data.showFilterButton, and data.showTotalsRow are optional. data.name must start with a letter or underscore, contain only letters/numbers/underscores, not start with true or false, not be R or C, and not be an A1/R1C1 cell reference. If the requested table behavior cannot be mapped exactly on the current host, return type=\"error\" with data.code=\"UNSUPPORTED_OPERATION\".",
    "If type=\"named_range_update\", data.operation, data.scope, data.name, data.explanation, data.confidence, and data.requiresConfirmation must be true. create and retarget must include targetSheet and targetRange. rename must include newName. delete must not invent a target range. data.name and data.newName must start with a letter or underscore, contain only letters/numbers/underscores, not start with true or false, not be R or C, and not be A1/R1C1 cell references.",
    ...
    "If type=\"sheet_update\", data.targetSheet, data.targetRange, data.operation, data.explanation, data.confidence, data.shape, and data.requiresConfirmation must be true. data.operation must be replace_range, set_formulas, set_notes, or mixed_update. Do not use append_rows; append transferred ranges with range_transfer_plan or insert explicit rows before a sheet_update. Any provided values, formulas, or notes matrix must match data.shape, targetRange must match the same rectangle, and sheet_update formula cells must be null or start with =.",
