...
export function buildHermesSpreadsheetRequestPrompt(request: HermesRequest): string {
  ...
  return [
    ...
    "If type=\"conditional_format_plan\", data.targetSheet, data.targetRange, data.explanation, data.confidence, data.requiresConfirmation, data.affectedRanges, data.replacesExistingRules, and data.managementMode are required. conditional_format_plan is distinct from range_format_update. clear_on_target must contain no rule payload. replace_all_on_target removes existing target rules before applying the new rule. Do not return a vague highlight plan with only explanation and ranges. Include a full contract-valid rule payload. Conditional format style supports backgroundColor, textColor, bold, italic, underline, and strikethrough only; use range_format_update for static numberFormat changes. number_compare values must be finite numbers. date_compare values must be valid YYYY-MM-DD literals. For color_scale, points are ordered minimum, optional midpoint, maximum: do not put max first, min last, or min/max at the midpoint. For row-highlighting logic driven by a status/breach/overdue column or by comparisons between columns, prefer ruleType=\"custom_formula\" with an exact formula instead of static row-by-row formatting. If the requested conditional formatting cannot be mapped exactly on the current host, return type=\"error\" with data.code=\"UNSUPPORTED_OPERATION\".",
