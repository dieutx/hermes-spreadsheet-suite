...
const DRY_RUN_SENSITIVE_LANGUAGE_PATTERN =
  /(?:^|\n)\s*at\s+(?:file:\/\/\/|\S+)|(?:^|[\s(["'=:])\/(?:root|home|Users|var|tmp|workspace|app|srv|etc|opt|mnt)\/|[A-Za-z]:\\|(?:^|[\s(["'=:])\\\\[^\s]+|\b(?:stack trace|traceback|TypeError|ReferenceError|SyntaxError|RangeError):?|https?:\/\/(?:localhost|127(?:\.\d{1,3}){3}|0\.0\.0\.0|10\.|169\.254\.\d{1,3}\.\d{1,3}|192\.168\.|172\.(?:1[6-9]|2\d|3[01])\.|[^/\s]*internal|[^/\s]*\.local)(?:[/:]|\s|$)/i;
...
const UNSAFE_PROOF_METADATA_PATTERN = /(?:APPROVAL_SECRET|HERMES_API_SERVER_KEY|HERMES_AGENT_API_KEY|HERMES_AGENT_BASE_URL|OPENAI_API_KEY|ANTHROPIC_API_KEY|stack trace|traceback|ReferenceError|TypeError|SyntaxError|RangeError)|(?:^|\s)\/(?:root|srv|home|tmp|var|opt|workspace|app|mnt|Users)\/[^\s]+|[A-Za-z]:\\[^\s]+|(?:^|[\s=:])\\\\[^\s]+|https?:\/\/(?:internal(?:[.\w-]*)?|localhost|127\.0\.0\.1|10\.\d{1,3}\.\d{1,3}\.\d{1,3}|169\.254\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|172\.(?:1[6-9]|2\d|3[0-1])\.\d{1,3}\.\d{1,3})[^\s]*/i;
