...
describe("shared client render helpers", () => {
  ...
  it("sanitizes sensitive raw text gateway errors from shared-client responses", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => new Response(
      String.raw`Gateway failed at path=\\runner\share\server.ts:42`,
      {
        status: 500,
        headers: {
          "content-type": "text/plain"
    ...
    await expect(client.pollRun("run_wrapped_posix")).rejects.toThrow(
      "Hermes gateway request failed with HTTP 500."
    );
  ...
  it("sanitizes sensitive JSON gateway errors from shared-client responses", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => new Response(JSON.stringify({
      error: {
        message: "Gateway failed for qa_HERMES_API_SERVER_KEY"
      }
    }), {
      status: 500,
      headers: {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => new Response(JSON.stringify({
      error: {
        ...
        userAction: String.raw`Inspect path=\\runner\share\debug.log before retrying.`
      }
    }), {
      status: 500,
      headers: {
    ...
    await expect(client.pollRun("run_wrapped_posix_json")).rejects.toThrow(
      "Hermes gateway request failed with HTTP 500."
    );
  });
  ...
  it("sanitizes standalone secret markers from dry-run unsupported reasons", () => {
    const dryRunResult = {
      simulated: false,
      planDigest: "digest_standalone_secret_marker",
      workbookSessionKey: "excel_windows::workbook-123",
      steps: [],
      predictedAffectedRanges: [],
      predictedSummaries: [],
      ...
      reversible: false,
      expiresAt: "2026-04-20T13:00:00.000Z",
      unsupportedReason: "Dry-run failed while reading TOKEN"
    } as any;
  ...
  it("redacts numeric private IPv4 proof identifiers in shared-client proof lines", () => {
    const response = baseResponse({
      requestId: "http://2130706433/debug",
      hermesRunId: "http://2852039166/debug"
    });

    const proof = formatProofLine(response);
    expect(proof).toContain("requestId unavailable");
    expect(proof).toContain("hermesRunId unavailable");
    expect(proof).not.toContain("2130706433");
    expect(proof).not.toContain("2852039166");
  });
  ...
  it("redacts unsafe proof metadata in shared-client meta lines", () => {
    const response = baseResponse({
      skillsUsed: [
        ...
        "https://2130706433/latest/meta-data",
    ...
    expect(metaLine).not.toContain("2130706433");
