...
describe("Hermes spreadsheet contracts", () => {
  ...
  it("rejects sheet structure range operations whose affectedRanges omit the target range", () => {
    const parsed = SheetStructureUpdateDataSchema.safeParse({
      targetSheet: "Sheet1",
      operation: "merge_cells",
      targetRange: "A1:C1",
      ...
      requiresConfirmation: true,
      confirmationLevel: "standard",
      affectedRanges: ["Sheet1!E1:G1"]
    });

    expect(parsed.success).toBe(false);
  });

  it("accepts sheet structure affectedRanges with equivalent absolute A1 refs", () => {
    const parsed = SheetStructureUpdateDataSchema.parse({
      targetSheet: "Sheet1",
      operation: "merge_cells",
      targetRange: "$A$1:$C$1",
      ...
      requiresConfirmation: true,
      confirmationLevel: "standard",
      affectedRanges: ["Sheet1!A1:C1"]
    });

    expect(parsed.affectedRanges).toEqual(["Sheet1!A1:C1"]);
  });
  ...
  it("accepts range sort affectedRanges with equivalent absolute A1 refs", () => {
    const parsed = RangeSortPlanDataSchema.parse({
      targetSheet: "Sheet1",
  ...
  it("accepts exact-safe pie chart plans with one series and no axes", () => {
    const parsed = ChartPlanDataSchema.parse({
      sourceSheet: "Sales",
      sourceRange: "A1:C20",
      targetSheet: "Sales Chart",
    ...
    expect(parsed.series).toEqual([{ field: "Revenue", label: "Revenue" }]);
  });
  ...
  it("accepts table affectedRanges with equivalent absolute A1 refs", () => {
    const parsed = TablePlanDataSchema.parse({
      targetSheet: "Sales",
      targetRange: "$A$1:$F$50",
      name: "SalesTable",
  ...
  it("rejects destructive confirmation on conditional format add plans", () => {
    const parsed = ConditionalFormatPlanDataSchema.safeParse({
      targetSheet: "Sheet1",
      targetRange: "B2:B20",
      managementMode: "add",
  ...
  it("rejects color scale points in impossible positions", () => {
    const invalidPointSets = [
      ...
      [
        { type: "min", color: "#f8696b" },
        { type: "max", color: "#ffeb84" },
        { type: "max", color: "#63be7b" }
      ],
      [
        { type: "number", value: 0, color: "#f8696b" },
        { type: "percentile", value: 50, color: "#ffeb84" },
